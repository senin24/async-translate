...
public class TranslateController {
  ...
  @ApiOperation(value = "Синхронный запрос перевода.")
  public ResponseEntity<TranslateResponse> translate(@Valid @RequestBody TranslateRequestDto request, HttpServletRequest servletRequest) {
    TranslateResponse translate =
        translatorService.translate(request.getTextFrom(), request.getFromLang(), request.getToLang(), servletRequest.getRemoteAddr());
  ...
  @ApiOperation(value = "Асинхронный запрос перевода. Возвращает адрес ресурса по которому можно найти результат перевода.")
  ...
  public ResponseEntity<UUID> createTranslateTaskAsync(@Valid @RequestBody TranslateRequestDto request, HttpServletRequest servletRequest) {
    ...
    translatorService.createTranslateTaskAsync(
        request.getTextFrom(), request.getFromLang(), request.getToLang(), servletRequest.getRemoteAddr(), id);
    URI uri = MvcUriComponentsBuilder.fromController(getClass()).path("/{id}").buildAndExpand(id).toUri();
    return ResponseEntity.created(uri).body(id);
  }

  @ApiOperation(value = "Поиск результата перевода по id.")
